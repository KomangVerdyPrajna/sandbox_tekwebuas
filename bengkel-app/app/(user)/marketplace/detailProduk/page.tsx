...
// Import ikon yang lebih banyak untuk highlights
import { Star, ShoppingCart, ArrowLeft, MessageSquare, Tag, Info, ShieldCheck, Truck, Clock, Zap } from "lucide-react";
...
const addToCart = (product: Product) => {
    ...
    // Menggunakan custom modal/message box
...
const FeatureHighlight = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
    <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
        <Icon size={24} className="text-[#234C6A] mt-1 flex-shrink-0" />
        <div>
    ...
    <div className="p-4 bg-white border border-gray-100 rounded-lg shadow-sm">
        <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex text-yellow-500">
                {Array(rating).fill("").map((_,idx)=><Star key={idx} size={18} fill="gold" stroke="gold"/>)}
                {Array(5-rating).fill("").map((_,idx)=><Star key={idx} size={18} fill="none" stroke="gold"/>)}
            </div>
        </div>
...
export default function ProductDetailPage() {
    ...
    const dummyReviews = [
        { name: "Pengguna A", rating: 5, comment: "Barang bagus, pengiriman cepat, harga sesuai! Mantap transaksi di sini 👍" },
        { name: "Siti K.", rating: 4, comment: "Oli sangat cocok untuk motor matic saya. Sayang pengiriman agak lambat." },
        { name: "Bambang M.", rating: 5, comment: "Helmnya keren dan nyaman dipakai. Benar-benar sesuai deskripsi!" },
    ];
    
    // Tambahkan Fitur Highlight Dummy
...
Selalu pastikan produk yang Anda beli sesuai dengan spesifikasi kendaraan Anda. Detail teknis lebih lanjut dapat dilihat pada kemasan produk.`
    ...
    if (loading) return (
        <div className="text-center py-20 text-[#234C6A] text-xl font-semibold">Memuat Detail Produk...</div>
    );
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-12 px-4 md:px-8">
            <div id="message-box" className="fixed top-4 right-4 z-50 transition-all duration-300 hidden"></div>
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid md:grid-cols-12 gap-10 bg-white p-8 lg:p-12 rounded-3xl shadow-[0_15px_30px_-15px_rgba(0,0,0,0.1)] border border-gray-100">
                    ...
                    {/* Kolom Kiri: Gambar Produk (Lebih Besar) */}
                    ...
                    <div className="md:col-span-7 space-y-6">
                        ...
                        <div className="space-y-2 border-b pb-4">
                            <p className="text-sm font-semibold text-[#FF6D1F] uppercase tracking-wider flex items-center gap-1">
                                ...
                                Kategori: <span className="text-gray-700 font-bold">{product.category.name}</span>
                            </p>
                        ...
                        {/* Tombol Aksi - Lebih Bold dan Efektif */}
                ...
                {/* ==================== DESKRIPSI PRODUK (DENGAN WHITE-SPACE PRE-WRAP) ==================== */}
