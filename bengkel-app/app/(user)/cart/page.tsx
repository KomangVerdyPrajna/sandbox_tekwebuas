...
export default function CartPage() {
  ...
  const updateQty = async (id:number, qty:number) => {
    ...
    playAnim(id); // efek animasi angka
    ...
    await fetch(`http://localhost:8000/api/cart/${id}`, {
      ...
      body:JSON.stringify({
        product_id: cart.find(c=>c.id===id)?.product.id,
        quantity: qty
      })
    });
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-5">
          {cart.map(item=>(
            <div key={item.id}
              className="flex bg-white p-5 rounded-xl shadow-md gap-6 items-center border 
            ...
            >
              ...
              <div className="w-24 h-24 rounded-lg overflow-hidden bg-gray-200 flex items-center justify-center border">
                <img
                  src={`http://localhost:8000/storage/${item.product.img_url}`}
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`
                    font-semibold text-[18px] w-6 text-center text-[#234C6A]
